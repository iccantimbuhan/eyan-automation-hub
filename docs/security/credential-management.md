...
| Ollama (Local) | Ollama (`ollamaApi`) | **Unused as of ADR-0014 (2026-08-13)** — previously `workflows/finance/02-finance-intent-router.json`'s `Ollama Chat Model` node | Backed the Finance Intent Router's pre-migration AI Agent (`baseUrl: http://host.docker.internal:11434`, no API key — Ollama has no auth in this local setup). The `Ollama Chat Model` node that referenced this credential was deleted by `docs/adrs/ADR-0014-finance-ai-core-migration.md`; no workflow in this repo references this credential anymore (confirmed by grepping every workflow JSON's `credentials` blocks). **Not yet deleted from n8n's credential store** — no supported CLI/API path was available to delete it safely in this session (n8n's CLI has no `delete:credentials` command); delete it manually via the n8n editor (Settings → Credentials → Ollama (Local) → Delete) whenever convenient. |
