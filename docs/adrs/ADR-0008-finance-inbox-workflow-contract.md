...
| `GET_BUDGET` | Answer "what's my budget" | `period?` (`YYYY-MM`, defaults to current) | **Not built** — no dedicated endpoint; satisfy via `GET_DASHBOARD`'s `budget`/`remainingBudget` fields until/unless a dedicated route is added (ADR-0024 Decision 2/Alternatives) |
| `GET_DASHBOARD` | Answer "show me my spending" | `period?` | **Full** — `GET /finance/service/dashboard` |
| `GET_FINANCE_QUESTION` | Open-ended NL finance question not covered by the two structured reads above | `question` (raw text), `impliedPeriod?`, `impliedCategory?` | **Full**, composed from `GET /finance/service/dashboard` + `GET /finance/service/categories` — no dedicated endpoint of its own |
