# CREATE_EXPENSE Handler — Expense Extraction Prompt (v2)
...
**v2 makes two changes.** First, it adds the `isRecurring` field: v1 extracted five fields, but this repository's Finance Handler verification pass found that `eyan-ai-platform`'s `CreateExpenseAutomatedDto` and its validator (`finance-automation.validator.ts`) already support an optional `isRecurring: boolean`, which `FinanceExpenseService.create()` uses to create a real `RecurringTemplate` row — a genuine, already-supported backend capability v1 simply never extracted or forwarded. Second, it removes the inline single backticks that v1 wrapped around `${today}` in the `date` field's instructions (`` use `${today}` ``) — those are valid Markdown styling in this reference file, but v1's *embedded* copy (inside `10-handle-create-expense.json`'s `Prepare Expense Extraction Input` node) placed the same backtick-wrapped text literally inside a JavaScript template literal, where a bare backtick closes the literal rather than nesting — a genuine syntax error that would have thrown the first time that Code node actually ran, caught during this repository's live-validation pass, never during logic-only testing. Both the `.md` reference copy and the embedded string are plain, backtick-free prose now.
...
Respond with ONLY a single JSON object with exactly these six keys (amount, category, paymentMethod, date, description, isRecurring) and no other text, no markdown formatting, no explanation.
